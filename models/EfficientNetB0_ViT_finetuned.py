...
class EfficientNetB0_ViT_Finetuned(nn.Module):
    """EfficientNetB0 + ViT variant prepared for conservative fine-tuning.

    Default training only updates the ViT aggregators and classifier head. Use
    unfreeze_last_backbone_blocks() or unfreeze_backbones() for later stages.
    """
    ...
    def unfreeze_last_backbone_blocks(self, num_blocks: int = 2):
        """Unfreeze the last EfficientNet feature blocks for staged fine-tuning."""
        self.freeze_backbones()
        num_blocks = max(1, int(num_blocks))
        for net in (self.axial, self.coronal, self.sagittal):
            for block in list(net.children())[-num_blocks:]:
